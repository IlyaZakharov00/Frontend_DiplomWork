...
export default function SliderTimeArrival() {
    ...
    useEffect(() => {
        if (state.times.start_arrival_hour_from !== undefined && state.times.start_arrival_hour_to) {
            setValueStart([state.times.start_arrival_hour_from, state.times.start_arrival_hour_to]);
        }
        if (state.times.end_arrival_hour_from !== undefined && state.times.end_arrival_hour_to) {
